...
- **Nightly sync**: `/api/cron/qb-sync` (Vercel cron, `vercel.json`) runs the whole import — jobs *and* financials — every morning at `QB_SYNC_HOUR` (default 4) local time in `QB_SYNC_TIMEZONE` (default `America/New_York`). It can't be one request: a single invocation caps at 300s, so migration `0024` models each morning as a **run** of ordered **steps** (`customers_jobs`, `job_costs`, then one `general_ledger` step per connected realm) and the route is a worker — every tick claims steps it can finish, later ticks drain the rest. Postgres owns the invariants (`begin_qb_sync_run` / `claim_qb_sync_step` / `finish_qb_sync_step`): one scheduled run per org per *local* date, at most one step running anywhere (steps rotate the QB refresh token — two at once can strand a connection), steps in `position` order, and stale-step revival with three attempts. Because Vercel cron is UTC-only the schedule is a UTC *window* and `localClock()` in `src/lib/qbSyncSchedule.ts` decides when the run starts, which keeps 4 AM at 4 AM across DST; move the `vercel.json` window if the hour/timezone changes. The route authenticates with `CRON_SECRET` and is exempt from the proxy's session redirect via `PUBLIC_PATHS`.
...
The dashboards' Benefit allocation column (Jobs and Capitalized Labor, plus their workbook exports) comes from the `job_benefit_allocation_months` view (migrations `0021`/`0022`; `job_benefit_allocation_totals` is its all-time/YTD/MTD rollup): per QB company × month it computes the Income Statement's employee-benefits allocation — Employee Benefits × Direct Labor ÷ (Direct Labor + Salaries & Wages), matched via `gl_accounts.category` and account names — and distributes it across that company's jobs pro-rata by direct-labor cost. Its matching/math is a deliberate SQL mirror of `allocateBenefits` in `src/lib/financials.ts` (same lockstep contract as `costing.ts`); the views are intentionally **not** `security_invoker` so they can aggregate the admin-only `gl_lines` while exposing only per-job dollars to all signed-in users. Deriving the allocation is far too slow for a page load — even one evaluation blows the authenticated role's statement timeout, and a bigger timeout is not available to browser-facing roles (`statement_timeout` is armed before a function's own `SET` applies, see migration `0012`) — so **nothing reads it live**: migration `0025` materializes the month view as `job_benefit_allocation_cache` and points both readers at it (`job_benefit_allocation_totals` for the Jobs dashboard; `job_benefit_allocation_summary(p_from, p_to)` from migration `0024`, which hands the Capitalized Labor page its per-job period sums and per-job × year split as one JSON row). The cache is rebuilt by `refresh_job_benefit_allocation()` — granted to `service_role` only, wrapped by `refreshBenefitAllocation()` in `src/lib/benefitAllocation.ts` — at the three moments its inputs move: a jobs/costs sync, a ledger sync, and an account's category changing. Anything else that can change those inputs must refresh it too, or the dashboards go stale.
