...
export function QbSyncButton() {
  ...
  async function sync() {
    ...
    try {
      ...
      setMessage(
        `Imported ${json.customers} customers, ${json.jobs} jobs, ${json.costLines ?? 0} cost lines, and ${json.invoices ?? 0} invoices from ${json.companies} ${json.companies === 1 ? "company" : "companies"}.`,
      );
      router.refresh();
    } catch (e) {
